...
The customer had already designed a physical board game of Don’t Panic, which formed the basis for designing the electronic version. In the earliest stages of the designing myBalsamiq Mockups was used, an online tool for interface designing.\\
\\
Balsamiq is designed to be an easy and efficient tool used in the early stages of interface designing, and it can be used to generate click-through prototypes for interfaces. Through myBalsamiq it can also be used as a collaborative tool, supporting project-based collaboration and real-time changes.\\
...
With the use of JSON all the game variables were stored in one string instead of storing it in the different database tables. Therefore the final database model is smaller than the initial ER diagram, but actually contains the same amount of variables as the initial ER diagram.	
\begin{figure}[H]
  \centering
